var background = {}
    ...
    $this.object = $("<div style='z-index:-1; margin:0; padding:0; overflow:hidden; position:absolute; bottom:0; background: linear-gradient(#A7D3EC,#BEF3FB);' id='"+$this.id+"'> </div>'").appendTo(".intro  ");
  ...
  background.generate_bubbles = function() {
   ...
   if(shape_type == 1) {
    var bolla = base.css({"background": "url(assets/Dashboard-mahasiswa/img/Green.png)"});
  }else if (shape_type == 2){
    var bolla = base.css({"background": "url(assets/Dashboard-mahasiswa/img/Orange.png)"});
  }else if (shape_type == 3){
    var bolla = base.css({"background": "url(assets/Dashboard-mahasiswa/img/Pink.png)"});
  }else{
    var bolla = base.css({"background": "url(assets/Dashboard-mahasiswa/img/Red.png)"});
  }    
